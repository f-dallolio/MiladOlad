...
pak::pkg_install("f-dallolio/loadr", ask = FALSE)

loadr::loadr(data.table,
             fastDummies,
             forecast,
             tidyverse,
             glue,
...
# data_dir <- "D:/My_Codes/Thesis/final_match/"
data_dir <- "~/Documents/R_wd/r_projects/milad/"


data_paths <- c(df_rms = file.path(data_dir, "r_top40_brands_week.csv"),
                df_adintel = file.path(data_dir, "A_Processed_For_Regression.csv"),
                df_programs = file.path(data_dir, "TV_Program_Categories.csv"),
                df_creatives = file.path(data_dir, "CreativeDescriptions_Coded.csv"),
                df_matches = file.path(data_dir, "T40_Final_BrandMatch.csv"))

data_list <- lapply(data_paths,
                    function(x) as_tibble(fread(file = x)))
...
df <- fread(glue("{final_match_dir}/A_Processed_For_Regression.csv")) |>
...
matches <- fread(glue("{final_match_dir}/T40_Final_BrandMatch.csv")) |>
  distinct(brand_code_uc, BrandCode)

rms <- fread(glue("{final_match_dir}/r_top40_brands_week.csv")) |>
